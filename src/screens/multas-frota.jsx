...
function MultasFrota() {
  const initialFilters = { q:"", placa:"", motorista:"", status:"aberta", indicacao:"todos", statusMotorista:"todos", statusMulta:"todos", vencimentoDe:"", vencimentoAte:"", ordenar:"placa", direcao:"asc" };
  const [data,setData]=useState({resumo:{},multas:[],paginacao:{page:1,pageSize:40,total:0,totalPages:1}});
  ...
  useEffect(()=>{ const timer=setTimeout(()=>{setPage(1);load(1);},350); return ()=>clearTimeout(timer); },[filters.q,filters.placa,filters.motorista,filters.status,filters.indicacao,filters.statusMotorista,filters.statusMulta,filters.vencimentoDe,filters.vencimentoAte,filters.ordenar,filters.direcao,pageSize]);
  ...
  return <div className="view multas-page">
    ...
    <div className="card multas-filtros"><div className="multas-filter-grid extended">
      ...
      <MultaFiltro label="Pagamento"><select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="todos">Todos os pagamentos ERP</option><option value="aberta">Em aberto</option><option value="vencida">Vencidas</option><option value="paga">Pagas</option></select></MultaFiltro>
