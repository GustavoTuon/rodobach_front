...
const vgPrevMonth = () => {
  ...
  const y = d.getFullYear(),
    ...
    last = new Date(y, d.getMonth() + 1, 0);
  return {
    s: `${y}-${m}-01`,
    e: `${last.getFullYear()}-${String(last.getMonth() + 1).padStart(2, "0")}-${String(last.getDate()).padStart(2, "0")}`,
  };
};
const VG_PERIODS = [
  ...
  { key: "7d", label: "7 dias", gr: () => ({ s: vgDaysAgo(6), e: vgToday() }) },
  {
    key: "30d",
    label: "30 dias",
    gr: () => ({ s: vgDaysAgo(29), e: vgToday() }),
  },
  {
    key: "mes",
    label: "Este mês",
    gr: () => ({ s: vgMonthStart(), e: vgToday() }),
  },
...
const FORM_EMPTY = {
  ...
  docs: {
    ...
    antt: false,
    cnh: false,
    consultaMotorista: false,
    contaDeposito: false,
    chavePix: false,
    comprovanteResidencia: false,
    numeroMotorista: false,
  },
  paradas: [],
...
const calcularSituacaoViagem = (input = {}) => {
  if (["em_transito", "entregue", "cancelado"].includes(input.situacao))
    return input.situacao;

  const hasCte = (
    Array.isArray(input.documentosFinanceiros)
      ? input.documentosFinanceiros
      : []
  ).some((doc) => {
    const tipo = textValue(doc?.tipo || doc?.tipoDocumento)
      .trim()
      .toUpperCase()
      .replace(/[^A-Z]/g, "");
    return (
      tipo === "CTE" &&
      (hasValue(doc?.numero || doc?.numeroDocumento) ||
        hasValue(doc?.chave || doc?.chaveDocumento))
    );
  });

  const ownershipType = normalizeVehicleOwnershipType(
    input.vehicleOwnershipType || input.ownershipType || input.tipoPropriedade,
  );
  const hasCarga = [
    input.cliente,
    input.material,
...
const normalizeParadas = (paradas = []) =>
  (Array.isArray(paradas) ? paradas : []).map((p, index) => ({
...
const normalizeDocumentosFinanceiros = (documentos = []) =>
...
const formatRoutePoint = (cidade, uf) =>
  [textValue(cidade).trim(), textValue(uf).trim().toUpperCase()]
    .filter(Boolean)
    .join(", ");
...
const normalizeMapsUrl = (value) => {
  ...
  if (!url) return "";
  if (
    /^https?:\/\/(www\.)?(google\.[^/]+\/maps|maps\.app\.goo\.gl|goo\.gl\/maps)/i.test(
      url,
    )
  )
    return url;
...
const normalizeViagemForm = (input = {}) => ({
  ...
  vehicleOwnershipType: normalizeVehicleOwnershipType(
  ...
  documentosFinanceiros: normalizeDocumentosFinanceiros(
    input.documentosFinanceiros,
  ),
...
const PrintSheet = ({ form }) => {
  const lucroVal =
    numericValue(form.valorCliente) - numericValue(form.valorMotorista);
  const money = (v) =>
    numericValue(v)
      ? `R$ ${fmtNum(numericValue(v), { minimumFractionDigits: 2 })}`
      : "—";
  ...
  const hasValores =
    numericValue(form.valorCliente) > 0 ||
    numericValue(form.valorMotorista) > 0;
    ...
    <div className="rb-print-sheet">
      ...
      >
        {[
          { label: "VALOR DA VIAGEM", value: money(form.valorCliente) },
          {
            ...
            value: hasValores ? money(lucroVal) : "—",
          },
          { label: "PEDÁGIO", value: "—" },
      ...
      >
        ...
        <span style={{ color: "#6b7280", fontWeight: 400 }}>
          CONFERIR ANTES DE ENCAMINHAR PARA FATURAMENTO
        </span>
      </div>

      {/* ── Duas colunas principais ── */}
      <div
        className="rb-print-data-grid"
        style={{
          display: "grid",
          gridTemplateColumns: "1fr 1fr",
          borderBottom: "1px solid #e5e7eb",
      ...
      >
        ...
        <div style={{ padding: "14px 16px", borderRight: "1px solid #e5e7eb" }}>
          ...
          <PRRow2
            left={{ label: "N° viagem", value: form.numero }}
            right={{ label: "Situação", value: sit }}
          />
          ...
          <PRRow2
            left={{ label: "Material", value: form.material }}
            right={{ label: "Peso", value: pesoFmt }}
          />
          <PRRow2
            left={{ label: "Valor cliente", value: money(form.valorCliente) }}
            right={{ label: "R$/kg", value: rsKg }}
          />
          <PRRow2
            left={{ label: "R$/ton", value: rsTon }}
            right={{ label: "KM da viagem", value: kmFmt }}
          />
          <PRRow2
            left={{ label: "Entregas", value: String(form.paradas.length) }}
            right={{ label: "Vendedor", value: form.vendedor }}
          />
        </div>
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          <PRRow2
            left={{ label: "Motorista", value: form.motorista }}
            right={{ label: "Placa do veículo", value: form.placa }}
          />
          <PRRow2
            left={{
              label: "Valor motorista",
              value: money(form.valorMotorista),
            }}
            right={{
              label: "Lucro previsto",
              value: hasValores ? money(lucroVal) : "—",
          ...
          <PRRow2
            ...
            right={{
              ...
              value: form.numeroMotorista,
            }}
          />
          <PRRow2
            left={{ label: "CNH do motorista", value: form.cnh }}
            right={{ label: "ANTT do veículo", value: form.antt }}
          />
          <PRRow2
            left={{ label: "Conta depósito", value: form.contaDeposito }}
            right={{ label: "Chave PIX", value: form.chavePix }}
          />
        </div>
      </div>
...
const APPROVAL = {
  ...
  aguardando_aprovacao: { label: "Aguardando Comercial", cls: "waiting" },
  aprovada: { label: "Aprovada", cls: "approved" },
  correcao_solicitada: { label: "Correção solicitada", cls: "correction" },
  reprovada: { label: "Reprovada", cls: "rejected" },
  cancelada: { label: "Cancelada", cls: "rejected" },
};
const AUDIT_FIELD_LABELS = {
  ...
  rota_maps_url: "Rota Maps",
  observacoes: "Observações",
};
const auditChanges = (item) =>
  Object.keys(AUDIT_FIELD_LABELS).filter(
    (key) =>
      String(item.dados_anteriores?.[key] ?? "") !==
      String(item.dados_novos?.[key] ?? ""),
  );

const Viagens = ({ onNavigate, user }) => {
  ...
  const [approvalAudit, setApprovalAudit] = useState(null);
  const [appliedFilters, setAppliedFilters] = useState(() => ({
    dataInicio: vgDaysAgo(29),
    dataFim: vgToday(),
    limit: 500,
  }));
  ...
  const [cidadeLoading, setCidadeLoading] = useState({
    origem: false,
    destino: false,
  });
    ...
    window.RB_API.listViagens(appliedFilters)
      .then((items) => {
        if (!Array.isArray(items)) {
          setViagensError("Resposta inesperada do servidor.");
          return;
        }
        setViagens(items);
      ...
      .catch((error) => {
        ...
        setViagensError(
          error?.message || "Não foi possível carregar as viagens.",
        );
      })
    ...
    console.table([
      {
        area: "binding",
        status: "corrigido",
        detalhe: "normalizacao evita null/undefined em inputs controlados",
      },
      {
        area: "paradas",
        status: "corrigido",
        detalhe: "updates funcionais evitam perda de dados entre etapas",
      },
      {
        area: "autocomplete",
        status: "corrigido",
        detalhe: "campos continuam editaveis apos preenchimento automatico",
      },
      {
        area: "readonly/disabled",
        status: "ok",
        detalhe:
          "nenhum input do cadastro ficou readonly ou disabled indevidamente",
      },
      {
        area: "salvamento",
        status: "auditado",
        detalhe:
          "payload enviado ao backend e retorno salvo aparecem no console",
      },
    ]);
    console.groupEnd();
  }, []);
    ...
    if (window.NT_SIM) {
      ...
      setForm(
        normalizeViagemForm({
          ...
          valorMotorista: sim.valorMotorista
  ...
  const debugField = (field, value, nextForm) => {
    if (!VIAGENS_FORM_DEBUG) return;
    console.debug("[Viagens][campo]", field, {
      digitado: value,
      formState: nextForm?.[field],
    });
  };
  ...
  const setDoc = (key, val) =>
    setForm((prev) => {
      const next = {
        ...prev,
        docs: { ...normalizeDocs(prev.docs), [key]: val },
      };
      if (VIAGENS_FORM_DEBUG)
        console.debug("[Viagens][documento]", key, {
          digitado: val,
          formState: next.docs[key],
        });
      return next;
    });
  const setParadas = (updater) =>
    setForm((prev) => {
      ...
      if (VIAGENS_FORM_DEBUG)
        console.debug("[Viagens][paradas]", {
          valorDigitado: nextParadas,
          formState: next.paradas,
        });
      return next;
    });
  ...
  const mergeOptionDetails = (key, items = []) => {
    ...
    setOpcoes((prev) => {
      ...
      const labels = items
        .map((item) => item.placa || item.nome || item.label)
        .filter(Boolean);
      return {
        ...prev,
        [key]: [...new Set([...(prev[key] || []), ...labels])].sort((a, b) =>
  ...
  const applyMotorista = (value) => {
    const item = (opcoes.detalhes?.motoristas || []).find(
      (m) =>
        String(m.nome || "").toLowerCase() ===
        String(value || "").toLowerCase(),
    );
    if (!item) return;
    if (VIAGENS_FORM_DEBUG)
      console.debug("[Viagens][autocomplete motorista]", {
        selecionado: value,
        preenchido: item,
      });
    setForm((prev) => ({
  ...
  const applyPlaca = (value) => {
    const clean = String(value || "")
      ...
      .replace(/[^a-z0-9]/gi, "")
      .toLowerCase();
    const item = (opcoes.detalhes?.placas || []).find(
      (p) =>
        String(p.placa || "")
          .replace(/[^a-z0-9]/gi, "")
          .toLowerCase() === clean,
    ...
    if (!item) return;
    if (VIAGENS_FORM_DEBUG)
      console.debug("[Viagens][autocomplete placa]", {
        selecionado: value,
        preenchido: item,
      });
    setForm((prev) => ({
      ...
      vehicleOwnershipType: normalizeVehicleOwnershipType(
        item.vehicleOwnershipType || item.ownershipType || item.tipoPropriedade,
      ),
  ...
  const searchAutocomplete = (type, value) => {
    ...
    calls[type]?.(q)
      .then((items) => mergeOptionDetails(type, items))
      .catch(() => {});
  };
    ...
    autoFieldRef.current = ({
      ...
      <div className="trip-autocomplete-field" style={style}>
        <input
          ...
          autoComplete="off"
        />
        {activeAutoRef.current === activeKey &&
          (loading || String(value || "").trim().length >= 2) && (
            <div className="trip-autocomplete-list">
              {loading && (
                <div className="trip-autocomplete-empty">Buscando...</div>
              )}
              {!loading &&
              ...
              {!loading &&
                ...
                options.length === 0 && (
                  <div className="trip-autocomplete-empty">
  ...
  const selectedVehicleOption = (() => {
    const clean = String(form.placa || "")
      .replace(/[^a-z0-9]/gi, "")
      .toLowerCase();
    if (!clean) return null;
    return (
      (opcoes.detalhes?.placas || []).find(
        (p) =>
          String(p.placa || "")
            .replace(/[^a-z0-9]/gi, "")
            .toLowerCase() === clean,
  ...
  const selectedVehicleName =
    form.veiculoNome || selectedVehicleOption?.veiculo || "";
  const visibleDocsLabels = isFleetVehicle ? [] : DOCS_LABELS;
  const visibleSteps = isFleetVehicle
    ? STEPS.filter((s) => s.n !== 3).map((s, index) => ({
        ...s,
        displayN: index + 1,
        label: s.n === 4 ? "Documentos / Finaliza\u00e7\u00e3o" : s.label,
      }))
    : STEPS.map((s, index) => ({ ...s, displayN: index + 1 }));
  const currentStepIndex = Math.max(
    0,
    visibleSteps.findIndex((s) => s.n === step),
  );
  const previousStep = visibleSteps[Math.max(0, currentStepIndex - 1)]?.n || 1;
  const nextStep =
    visibleSteps[Math.min(visibleSteps.length - 1, currentStepIndex + 1)]?.n ||
    step;
  ...
  const openEdit = (v) => {
    ...
    if (VIAGENS_FORM_DEBUG)
      console.debug("[Viagens][editar] form carregado", normalized);
    setForm(normalized);
    setStep(1);
    setMode("form");
  };
  ...
  const deleteViagem = async (id) => {
    try {
      ...
      console.warn(
        "Nao foi possivel excluir no backend. Removendo localmente.",
        error,
      );
    }
  ...
  const persistForm = async () => {
    ...
    try {
      ...
      setViagens((prev) => {
        if (payload.id)
          return prev.map((v) => (v.id === payload.id ? saved : v));
        return [...prev, saved];
      });
      if (VIAGENS_FORM_DEBUG)
        console.debug("[Viagens][salvar] retorno backend", saved);
  ...
  const saveDocuments = async () => {
    ...
    try {
      const saved = await persistForm();
      if (saved) setDocumentsSaved(true);
    } finally {
      setDocSaving(false);
    }
  };
  ...
  const removeDocumentoFinanceiro = (id) =>
    setF(
      "documentosFinanceiros",
      normalizeDocumentosFinanceiros(form.documentosFinanceiros).filter(
        (doc) => doc.id !== id,
      ),
    );
  const setDocumentoFinanceiro = (id, field, value) =>
    setF(
      "documentosFinanceiros",
      normalizeDocumentosFinanceiros(form.documentosFinanceiros).map((doc) =>
  ...
  const pesquisarDocumentoFinanceiro = async () => {
    ...
    try {
      ...
      setDocResults([]);
      setDocSearchError(
        error?.message || "Nao foi possivel pesquisar os documentos.",
      );
    } finally {
  ...
  const approvalAction = async (viagem, acao) => {
    let motivo = "";
    if (["corrigir", "reprovar", "reabrir", "cancelar"].includes(acao)) {
      motivo = window.prompt("Informe a justificativa desta ação:") || "";
    ...
    if (
      !window.confirm(
        `Confirma ${labels[acao] || acao} a viagem ${viagem.numero || viagem.id}?`,
      )
    )
      return;
    setApprovalBusy(viagem.id);
    try {
      const saved = await window.RB_API.updateViagemAprovacao(viagem.id, {
        acao,
        motivo,
      });
      setViagens((current) =>
  ...
  const openApprovalAudit = async (viagem) => {
    setApprovalAudit({ viagem, loading: true, items: [], error: "" });
    try {
      const data = await window.RB_API.getViagemAuditoria(viagem.id);
      setApprovalAudit({
        viagem,
        loading: false,
  ...
  const vincularDocumentoFinanceiro = (documento) => {
    ...
    const tipo = textValue(documento.tipo || "CT-e") || "CT-e";
    const numero = textValue(documento.numero);
    const chave = textValue(documento.chave);
    if (
      atuais.some(
        (doc) =>
          (chave && doc.chave === chave) ||
          (doc.tipo === tipo && numero && doc.numero === numero),
      )
    )
      return;
    const next = [
    ...
    setF("documentosFinanceiros", next);
  };
  const documentoFinanceiroJaVinculado = (documento, tipoPadrao = "CT-e") => {
    const tipo = textValue(documento?.tipo || tipoPadrao);
    const numero = textValue(documento?.numero);
    const chave = textValue(documento?.chave);
    return normalizeDocumentosFinanceiros(form.documentosFinanceiros).some(
      (doc) =>
        (chave && doc.chave === chave) ||
        (doc.tipo === tipo && numero && doc.numero === numero),
    );
  };
  const cteComNotasJaVinculado = (documento) => {
    const notas = documento.notasDocumentos?.length
      ? documento.notasDocumentos
      : (documento.notas || []).map((numero, index) => ({
          numero,
          chave: documento.chavesNfe?.[index] || "",
        }));
    return (
      documentoFinanceiroJaVinculado(documento, "CT-e") &&
      notas.every((nota) => documentoFinanceiroJaVinculado(nota, "NF-e"))
    );
  };
  const vincularCteComNotas = (documento) => {
    const atuais = normalizeDocumentosFinanceiros(form.documentosFinanceiros);
    const candidates = [
      {
        id: `local-${Date.now()}-cte`,
        tipo: "CT-e",
      ...
      ...(documento.notasDocumentos?.length
        ? documento.notasDocumentos
        : (documento.notas || []).map((numero, index) => ({
            numero,
            chave: documento.chavesNfe?.[index] || "",
        ...
        id: `local-${Date.now()}-nf-${index}`,
        tipo: "NF-e",
        numero: String(nota.numero || ""),
        chave: nota.chave || "",
        link: "",
        observacoes: `NF-e vinculada ao CT-e ${documento.serie || ""}-${documento.numero || ""}`,
      })),
    ];
    const unique = [...atuais];
    for (const candidate of candidates) {
      if (
        !unique.some(
          (doc) =>
            (candidate.chave && doc.chave === candidate.chave) ||
            (doc.tipo === candidate.tipo && doc.numero === candidate.numero),
        )
      )
        unique.push(candidate);
    }
    setF("documentosFinanceiros", unique);
  };
  ...
  const filteredViagens = React.useMemo(() => {
    let list = viagens;
    if (filtroSit !== "todos")
      list = list.filter((v) => v.situacao === filtroSit);
    if (search) {
      const q = search.toLowerCase();
      list = list.filter((v) => {
        const notas = (v.paradas || [])
          .map((p) =>
            [p.nf, p.cliente, p.cidade, p.obs].filter(Boolean).join(" "),
          )
          .join(" ");
        return [
          v.numero,
    ...
    return [...list].sort((a, b) => {
      if (sortCol === "data")
        return dir * ((a.data || "") < (b.data || "") ? -1 : 1);
      if (sortCol === "cliente")
        return dir * ((a.cliente || "") < (b.cliente || "") ? -1 : 1);
      if (sortCol === "valorCliente")
        return dir * ((a.valorCliente || 0) - (b.valorCliente || 0));
      if (sortCol === "margem") {
        const ma =
          a.valorCliente > 0
            ? (a.valorCliente - a.valorMotorista) / a.valorCliente
            : -1;
        const mb =
          b.valorCliente > 0
            ? (b.valorCliente - b.valorMotorista) / b.valorCliente
            : -1;
        return dir * (ma - mb);
      }
      return 0;
  ...
  const margem = (v) =>
    v.valorCliente > 0 && v.valorMotorista > 0
      ? (((v.valorCliente - v.valorMotorista) / v.valorCliente) * 100).toFixed(
          1,
        )
      : null;
  ...
  if (mode === "list") {
    return (
      <div className="view">
          ...
          <div>
            ...
            <div className="sub">
              {loadingViagens
                ...
                : viagensError
                  ? "Erro ao carregar"
                  : `${viagens.length} viagens registradas · ${viagens.filter((v) => v.situacao === "em_transito").length} em trânsito`}
            </div>
          </div>
          ...
          >
            {loadingViagens ? (
              <span className="muted" style={{ fontSize: 12.5 }}>
                Carregando viagens do banco de dados…
              </span>
            ) : (
              <div>
                <div
                ...
                <div style={{ fontSize: 12, color: "var(--text-3)" }}>
                  ...
                  e reinicie o servidor backend.
                  {viagensError.toLowerCase().includes("password") &&
                    " A senha do banco pode ter expirado ou mudado."}
                  {(viagensError.toLowerCase().includes("table") ||
                    viagensError.toLowerCase().includes("relation")) &&
                    " Execute POST /api/admin/migrate para criar as tabelas."}
                </div>
              </div>
            )}
          </div>
        ...
        <div
          className="card"
          style={{ marginBottom: 14, padding: "14px 18px" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 7 }}>
              <button className="btn" onClick={clearVgFilters}>
                <Icon name="x" size={11} /> Limpar
              </button>
              <button className="btn primary" onClick={applyVgFilters}>
        ...
        {(() => {
          const valorTotal = viagens.reduce(
            (s, v) => s + (v.valorCliente || 0),
            0,
          );
          const valorMotTotal = viagens.reduce(
            (s, v) => s + (v.valorMotorista || 0),
            0,
          );
          ...
          return (
            <div className="grid cols-4" style={{ marginBottom: 14 }}>
              ...
              <KPI
                label="Receita total"
                icon="trending-up"
                value={`R$ ${fmtNum(valorTotal, { minimumFractionDigits: 0 })}`}
              />
              <KPI
                label="Lucro previsto"
                icon="money"
                value={`R$ ${fmtNum(lucroTotal, { minimumFractionDigits: 0 })}`}
                delta={
                  valorTotal > 0
                    ? `${((lucroTotal / valorTotal) * 100).toFixed(1)}% margem`
                    : ""
                }
              ...
              <KPI
                ...
                icon="truck"
                value={
                  viagens.filter((v) => v.situacao === "em_transito").length
                }
        ...
        <div className="tbl-toolbar" style={{ marginBottom: 0 }}>
          <div className="search">
            <Icon name="search" />
            <input
              placeholder="Buscar por numero, cliente, motorista, placa, rota, carga ou NF..."
              value={search}
              onChange={(e) => {
        ...
        {(() => {
          const totalPages = Math.max(
            1,
            Math.ceil(filteredViagens.length / PAGE_SIZE),
          );
          const pageRows = filteredViagens.slice(
            tablePage * PAGE_SIZE,
            ...
            <div className="card card-flush" style={{ marginTop: 10 }}>
              ...
              >
                <span className="muted" style={{ fontSize: 12 }}>
                  {filteredViagens.length} viagem
                  {filteredViagens.length !== 1 ? "s" : ""} encontrada
              ...
              <table className="tbl">
                <thead>
                  <tr>
                    ...
                    >
                      Cliente <VgSortArrow col="cliente" />
                    </th>
                    <th>Rota</th>
                    <th>Material</th>
                    ...
                    <th
                      className="num"
                      style={{ cursor: "pointer" }}
                      onClick={() => toggleSort("valorCliente")}
                    >
                      Vr. Cliente <VgSortArrow col="valorCliente" />
                    </th>
                ...
                <tbody>
                  ...
                  {pageRows.map((v) => {
                    const sit =
                      SITUACOES[v.situacao] || SITUACOES["faltando_dados"];
                    const approval =
                      APPROVAL[v.statusAprovacao] || APPROVAL.rascunho;
                    const isApprover = Boolean(
                      user?.admin || user?.permissions?.["aprovar-viagens"],
                    );
                    const mg = margem(v);
                    return (
                      <tr
                        key={v.id}
                        className={`clickable${v.situacao === "cancelado" ? " row-crit" : ["faltando_dados", "aguardando_veiculo"].includes(v.situacao) ? " row-warn" : ""}`}
                        onClick={() => openEdit(v)}
                      >
                        <td className="date" style={{ whiteSpace: "nowrap" }}>
                          {v.data
                            ? new Date(v.data + "T12:00:00").toLocaleDateString(
                                "pt-BR",
                                {
                                  day: "2-digit",
                                  month: "2-digit",
                        ...
                        <td>
                          <div style={{ fontSize: 12.5 }}>
                            {v.origem || "—"}{" "}
                            <span className="muted">({v.ufOrigem})</span>
                          </div>
                          <div className="muted" style={{ fontSize: 11.5 }}>
                            → {v.destino || "—"} <span>({v.ufDestino})</span>
                          </div>
                        </td>
                        <td
                          style={{
                        ...
                        <td>
                          <div style={{ fontSize: 12.5 }}>
                            {v.motorista || <span className="muted">—</span>}
                          </div>
                          {v.placa && <Plate value={v.placa} />}
                        </td>
                        <td className="num">
                          {v.valorCliente ? (
                            `R$ ${fmtNum(v.valorCliente, { minimumFractionDigits: 2 })}`
                          ) : (
                        ...
                        <td className="num">
                          {v.valorMotorista ? (
                            `R$ ${fmtNum(v.valorMotorista, { minimumFractionDigits: 2 })}`
                          ) : (
                            <span className="muted">—</span>
                          )}
                        </td>
                        ...
                        <td onClick={(e) => e.stopPropagation()}>
                          <button
                            type="button"
                            className={`vg-approval ${approval.cls}`}
                            title="Abrir histórico de aprovação"
                            onClick={() => openApprovalAudit(v)}
                          >
                            {approval.label}
                          </button>
                          {v.aprovadoPor &&
                            v.statusAprovacao === "aprovada" && (
                              <small className="vg-approved-by">
                                por {v.aprovadoPor}
                              </small>
                            )}
                          {v.atualizadoPor && (
                        ...
                        <td onClick={(e) => e.stopPropagation()}>
                          ...
                          >
                            {[
                              ...
                              "correcao_solicitada",
                              "reprovada",
                            ].includes(v.statusAprovacao || "rascunho") && (
                              <button
                                className="icon-btn vg-approval-action"
                                disabled={approvalBusy === v.id}
                                title="Enviar ao Comercial"
                                onClick={() => approvalAction(v, "enviar")}
                              >
                                <Icon name="check" size={13} />
                              </button>
                            )}
                            {isApprover &&
                              v.statusAprovacao === "aguardando_aprovacao" && (
                                <>
                                  <button
                                    className="icon-btn vg-approval-action approve"
                                    disabled={approvalBusy === v.id}
                                    title="Aprovar"
                                    onClick={() => approvalAction(v, "aprovar")}
                                  >
                                    <Icon name="check" size={13} />
                                  </button>
                                  <button
                                    className="icon-btn vg-approval-action correct"
                                    disabled={approvalBusy === v.id}
                                    title="Solicitar correção"
                                    onClick={() =>
                                      approvalAction(v, "corrigir")
                                    }
                                  >
                            ...
                            {isApprover && v.statusAprovacao === "aprovada" && (
                              <button
                                className="icon-btn vg-approval-action correct"
                                disabled={approvalBusy === v.id}
                                title="Reabrir aprovação"
                                onClick={() => approvalAction(v, "reabrir")}
                              >
              ...
              {totalPages > 1 && (
                <div className="tbl-footer">
                  <span className="muted" style={{ fontSize: 12 }}>
                    Pág. {tablePage + 1}/{totalPages} · {filteredViagens.length}{" "}
                    viagens
                  </span>
                  <div className="pager">
                    <button
        ...
        {approvalAudit && (
          <div
            className="vg-audit-overlay"
            onClick={(e) =>
              e.target === e.currentTarget && setApprovalAudit(null)
            }
          >
            <section className="card vg-audit-panel">
              <div className="vg-audit-head">
                <div>
                  <small>AUDITORIA COMERCIAL</small>
                  <h3>
                    {approvalAudit.viagem.numero ||
                      `Viagem ${approvalAudit.viagem.id}`}
                  </h3>
                </div>
                <button
                  className="icon-btn"
                  onClick={() => setApprovalAudit(null)}
                >
                  <Icon name="x" />
                </button>
              </div>
              {approvalAudit.loading ? (
                <div className="mp-page-state">Carregando histórico...</div>
              ) : approvalAudit.error ? (
                <div className="mp-alert error">{approvalAudit.error}</div>
              ) : (
                <div className="vg-audit-list">
                  {approvalAudit.items.map((item) => (
                    <article key={item.id}>
                      <i />
                      <div>
                        <strong>
                          {{
                            ...
                            enviar: "Enviado ao Comercial",
                            aprovar: "Aprovado",
                            corrigir: "Correção solicitada",
                            reprovar: "Reprovado",
                            reabrir: "Aprovação reaberta",
                            cancelar: "Cancelado",
                          }[item.acao] || item.acao}
                        </strong>
                        <span>
                  ...
                  {!approvalAudit.items.length && (
                    <div className="mp-empty">Nenhum evento registrado.</div>
                  )}
                </div>
              )}
            </section>
      ...
      <div className="view rb-print-view">
        ...
        >
          <div>
            ...
            <div className="sub">
              Pronto para imprimir e enviar ao financeiro.
            </div>
          </div>
          <div className="actions">
  ...
  const renderStep1 = () => (
    ...
    >
      ...
      <div className="col" style={{ gap: 14 }}>
          ...
          <AutoField
            ...
            renderOption={(item) => (
              <>
                ...
                <b>
                  {[
                    item.vehicleOwnershipType || item.ownershipType,
                    item.motorista || item.modelo || item.cidadePlaca,
                  ]
                    .filter(Boolean)
                    .join(" - ")}
                </b>
        ...
        <Fg label="Situação">
          {(() => {
            const situacaoCalculada = calcularSituacaoViagem({
              ...form,
              vehicleOwnershipType,
            });
            const sit =
              SITUACOES[situacaoCalculada] || SITUACOES.faltando_dados;
            return (
                  ...
                  >
                    ...
                    >
                      <span
                        style={{
                          fontWeight: 700,
                          color: isFleetVehicle ? "#047857" : "#b45309",
                        }}
                      >
                        {isFleetVehicle
                          ? "Ve\u00edculo Frota"
                      ...
                      {selectedVehicleName && (
                        <span className="muted">{selectedVehicleName}</span>
                      )}
                    </div>
                    {isFleetVehicle && (
                      <div
      ...
      <div className="col" style={{ gap: 0 }}>
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 10.5, fontWeight: 700 }}>
                {Math.max(routePoints.length - 2, 0)} par.
              </span>
            </div>
            ...
            >
              <span className="muted" style={{ fontSize: 11.5 }}>
                {routePoints.length >= 2
                  ? `${routePoints.length} pontos na rota`
                  : "Informe origem e destino para montar a rota"}
              </span>
              <button
                type="button"
                className="btn"
          ...
          <Fg label="Cidade de Origem">
            <div className="row" style={{ gap: 8 }}>
              <AutoField
                ...
                optionKey={(city, index) =>
                  city.codigo || `${city.nome}-${city.uf}-${index}`
                }
                onChange={(value) => {
                  setF("origem", value);
                  setF("ufOrigem", "");
              ...
              />
              <input
                style={{ ...fs, flex: 1, textAlign: "center" }}
                value={form.ufOrigem}
                onChange={(e) =>
                  setF("ufOrigem", e.target.value.toUpperCase().slice(0, 2))
                }
                placeholder="UF"
                maxLength="2"
              />
            ...
            {form.origem && (
              <button
                type="button"
                className="btn ghost"
                style={{ marginTop: 7, padding: "4px 8px", fontSize: 11.5 }}
                onClick={() =>
                  window.open(
                    buildGoogleMapsSearchUrl(form.origem, form.ufOrigem),
                    "_blank",
                    "noopener,noreferrer",
                  )
                }
              >
                <Icon name="map" size={11} /> Ver origem no Google Maps
              </button>
            )}
          </Fg>
          ...
          <Fg label="Cidade de Destino">
            <div className="row" style={{ gap: 8 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setF("ufDestino", e.target.value.toUpperCase().slice(0, 2))
                }
                placeholder="UF"
                maxLength="2"
              />
            ...
            {form.destino && (
              <div className="row between" style={{ marginTop: 7, gap: 8 }}>
                <span className="muted" style={{ fontSize: 11 }}>
                  Destino preenchido pelo cliente final quando disponível.
                </span>
                <button
                  type="button"
                  className="btn ghost"
                  ...
                  onClick={() =>
                    window.open(
                      buildGoogleMapsSearchUrl(form.destino, form.ufDestino),
                      "_blank",
                      "noopener,noreferrer",
                    )
                  }
                >
                  <Icon name="map" size={11} /> Ver destino
                </button>
              </div>
            )}
          </Fg>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div
            style={{
              maxHeight: 360,
              overflowY: form.paradas.length > 3 ? "auto" : "visible",
              paddingRight: form.paradas.length > 3 ? 4 : 0,
            }}
          >
            {form.paradas.map((p, i) => (
              <div
                ...
                >
                  <Fg label="Tipo">
                    <select
                      style={fs}
                      value={p.tipo}
                      onChange={(e) => setParada(p.id, "tipo", e.target.value)}
                    >
                  ...
                  <Fg label="N° Nota Fiscal">
                    <input
                      style={fs}
                      value={p.nf}
                      onChange={(e) => setParada(p.id, "nf", e.target.value)}
      ...
      <div className="col" style={{ gap: 14 }}>
        ...
        <Fg label="Cliente final (onde a carga será entregue)">
          <AutoField
            ...
            options={filterAutocompleteOptions(
              opcoes.detalhes?.clientes?.length
                ? opcoes.detalhes.clientes
                : (opcoes.clientesFinais || opcoes.clientes || []).map((c) => ({
                    nome: c,
                  })),
              form.clienteFinal,
            )}
        ...
        <Fg label="Tomador do serviço (quem vai pagar)">
          <AutoField
            ...
            options={filterAutocompleteOptions(
              opcoes.detalhes?.clientes?.length
                ? opcoes.detalhes.clientes
                : (opcoes.tomadores || opcoes.clientes || []).map((c) => ({
                    nome: c,
                  })),
              form.tomadorServico,
            )}
        ...
        <Fg label="Condição de Pagamento">
          <input
            style={fs}
            value={form.condicaoPagamento}
            onChange={(e) => setF("condicaoPagamento", e.target.value)}
            placeholder="Ex: 30 dias, à vista, 10/20/30..."
          />
        </Fg>
      </div>
      <div className="col" style={{ gap: 14 }}>
          ...
          <Fg label="Peso (kg)">
            <div className="row" style={{ gap: 6, alignItems: "center" }}>
              <input
                ...
                value={form.peso}
                onChange={(e) =>
                  setF("peso", sanitizeIntegerInput(e.target.value))
                }
          ...
          <Fg label="KM da viagem (opcional)">
            <div className="row" style={{ gap: 6, alignItems: "center" }}>
              <input
                ...
                value={form.km ?? ""}
                onChange={(e) =>
                  setF("km", sanitizeIntegerInput(e.target.value))
                }
                placeholder="Ex: 850"
              />
              <span className="muted" style={{ fontSize: 12, flexShrink: 0 }}>
        ...
        >
          ...
          >
            <Fg label="Valor do Cliente (R$)">
              <div className="row" style={{ gap: 6, alignItems: "center" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setF("valorCliente", sanitizeMoneyInput(e.target.value))
                  }
                  placeholder="0,00"
                />
              </div>
            ...
            <div style={{ paddingTop: 20 }}>
              {numericValue(form.valorCliente) > 0 &&
                numericValue(form.peso) > 0 && (
                  <div className="muted" style={{ fontSize: 12 }}>
                    R$/ton:{" "}
                    {fmtNum(
                      numericValue(form.valorCliente) /
                        (numericValue(form.peso) / 1000),
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div className="col" style={{ gap: 14 }}>
        ...
        <Fg label="Celular / Número do motorista">
          <input
            style={fs}
            value={form.numeroMotorista}
            onChange={(e) => setF("numeroMotorista", e.target.value)}
        ...
        <Fg label="CNH">
          <input
            style={fs}
            value={form.cnh}
            onChange={(e) => setF("cnh", e.target.value)}
      ...
      <div className="col" style={{ gap: 14 }}>
        <Fg label="Conta para depósito">
          <input
            style={fs}
            value={form.contaDeposito}
            onChange={(e) => setF("contaDeposito", e.target.value)}
            placeholder="Banco · C/C 0000-0"
          />
        </Fg>
        <Fg label="Chave Pix">
          <input
            style={fs}
            value={form.chavePix}
            onChange={(e) => setF("chavePix", e.target.value)}
        ...
        >
          ...
          <Fg label="Valor do Motorista (R$)">
            <div className="row" style={{ gap: 6, alignItems: "center" }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setF("valorMotorista", sanitizeMoneyInput(e.target.value))
                }
                placeholder="0,00"
              />
            </div>
          ...
          {numericValue(form.valorCliente) > 0 &&
            numericValue(form.valorMotorista) > 0 && (
              <div
                className="row"
                style={{ gap: 16, marginTop: 10, fontSize: 12.5 }}
              >
                <span className="muted">
                  ...
                  <b
                    ...
                    style={{
                      ...
                      color:
                        ((numericValue(form.valorCliente) -
                          numericValue(form.valorMotorista)) /
                          numericValue(form.valorCliente)) *
                          100 >=
                        20
                          ? "#047857"
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <div className="col" style={{ gap: 10 }}>
          ...
          visibleDocsLabels.map((doc) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ flex: 1, fontWeight: on ? 500 : 400 }}>
                  {doc.label}
                </span>
                {on && (
                  <Icon name="check" size={14} style={{ color: "#047857" }} />
                )}
              </div>
            );
          })}
        {!isFleetVehicle && (
          ...
          >
            <span className="muted">
              {Object.values(form.docs).filter(Boolean).length}
            </span>{" "}
            de {visibleDocsLabels.length} documentos recebidos
          </div>
        )}
        {isFleetVehicle && (
          <div
          ...
          >
            <b style={{ color: "#047857" }}>
              {"Ve\u00edculo de frota selecionado."}
            </b>{" "}
            {
              "Os documentos de motorista/terceiro n\u00e3o s\u00e3o necess\u00e1rios nesta viagem."
            }
          </div>
        )}
        <div
        ...
        >
          ...
          >
            ...
            <span
              style={{
                ...
                padding: "3px 8px",
                color:
                  isFleetVehicle || consultaMotoristaOk ? "#047857" : "#b45309",
                background:
                  isFleetVehicle || consultaMotoristaOk ? "#ecfdf5" : "#fffbeb",
          ...
          >
            ...
            >
              {hasCustomMapsRoute
                ? "O botao vai abrir a rota colada neste campo."
                : "Sem link colado, o botao monta a rota pela origem, paradas e destino."}
            </div>
              ...
              {routePoints.length > 0 ? (
                routePoints.map((point, index) => (
                  ...
                  >
                    <span
                      className="num"
                      style={{ width: 18, color: "var(--brand-blue)" }}
                    >
                      {index + 1}
                    </span>
        ...
        >
          ...
          >
            <div>
              ...
              <div className="muted" style={{ fontSize: 11.5 }}>
                Vincule CT-e, MDF-e, NF ou outro documento emitido.
              </div>
            </div>
            <button
              className="btn"
              type="button"
              onClick={addDocumentoFinanceiro}
            >
          ...
          >
            ...
            <div className="row" style={{ gap: 8 }}>
              ...
              <button
                className="btn"
                type="button"
                disabled={docSearching || docSearch.trim().length < 2}
                onClick={pesquisarDocumentoFinanceiro}
              >
            ...
            {docSearchError && (
              <div
                style={{ fontSize: 11.5, color: "var(--danger)", marginTop: 6 }}
              >
                {docSearchError}
              </div>
            )}
            {!!docResults.length && (
              <div className="col" style={{ gap: 6, marginTop: 8 }}>
                {docResults.map((doc, index) => {
                  const cteVinculado = documentoFinanceiroJaVinculado(
                    doc,
                    "CT-e",
                  );
                  const cteComNotasVinculado = cteComNotasJaVinculado(doc);
                  return (
                    <div
                      key={`${doc.serie}-${doc.numero}-${index}`}
                      className="vg-doc-result"
                    >
                      <span>
                        <strong>
                          CT-e {doc.serie}-{doc.numero}
                        </strong>
                        <br />
                        <small>
                          {doc.cliente} · {doc.placa || "Sem placa"}
                          {doc.notas?.length
                            ? ` · NF ${doc.notas.join(", ")}`
                            : ""}
                        </small>
                      ...
                      <div>
                        <button
                          type="button"
                          className="btn"
                          disabled={cteVinculado}
                          onClick={() => vincularDocumentoFinanceiro(doc)}
                        >
                          {cteVinculado ? "CT-e vinculado" : "Vincular CT-e"}
                        </button>
                        {!!doc.notas?.length && (
                          <button
                            type="button"
                            className="btn btn-primary"
                            disabled={cteComNotasVinculado}
                            onClick={() => vincularCteComNotas(doc)}
                          >
                            {cteComNotasVinculado
                              ? `CT-e + ${doc.notas.length} NF${doc.notas.length > 1 ? "s" : ""} vinculados`
                              : `CT-e + ${doc.notas.length} NF${doc.notas.length > 1 ? "s" : ""}`}
                          </button>
                        )}
                      </div>
                    </div>
            ...
            {!docSearching &&
              ...
              !docSearchError && (
                <div className="muted" style={{ fontSize: 11.5, marginTop: 6 }}>
                  Pesquise para localizar CT-e, NF ou chave no ERP.
                </div>
              )}
          </div>

          {normalizeDocumentosFinanceiros(form.documentosFinanceiros).length ===
            0 && (
          ...
          <div className="col" style={{ gap: 10 }}>
            {normalizeDocumentosFinanceiros(form.documentosFinanceiros).map(
              (doc, index) => (
                <div
                  key={doc.id}
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    <button
                      className="icon-btn"
                      type="button"
                      onClick={() => removeDocumentoFinanceiro(doc.id)}
                  ...
                  >
                    <Fg label="Tipo">
                      <select
                        ...
                        value={doc.tipo}
                        onChange={(e) =>
                          setDocumentoFinanceiro(doc.id, "tipo", e.target.value)
                        }
                      >
                  ...
                  <Fg label="Chave / Código">
                    <input
                      ...
                      value={doc.chave}
                      onChange={(e) =>
                        setDocumentoFinanceiro(doc.id, "chave", e.target.value)
                      }
                      placeholder="Chave de acesso ou código do documento"
                    />
                  </Fg>
                  <div style={{ height: 8 }} />
                  <Fg label="Link do documento">
                    <input
                      ...
                      value={doc.link}
                      onChange={(e) =>
                        setDocumentoFinanceiro(doc.id, "link", e.target.value)
                      }
                      placeholder="Cole o link do PDF/XML ou sistema"
                    />
                  </Fg>
                  <div style={{ height: 8 }} />
                  <Fg label="Observação">
          ...
          <div className="col" style={{ gap: 7 }}>
            {[
              { l: "N°", v: form.numero },
              {
                l: "Data",
                v: form.data
                  ? new Date(form.data + "T12:00:00").toLocaleDateString(
                      "pt-BR",
                    )
                  : "",
              },
              {
                ...
                v:
                  form.origem && form.destino
                    ? `${form.origem} (${form.ufOrigem}) → ${form.destino} (${form.ufDestino})`
                    : "",
              },
              { l: "Placa", v: form.placa },
              {
                l: "KM",
                v: form.km ? `${fmtNum(numericValue(form.km))} km` : "",
              },
              {
                l: "Paradas",
                v:
                  routePoints.length > 2 ? String(routePoints.length - 2) : "0",
              },
              {
                l: "Docs financeiro",
                v: String(
              ...
              },
              {
                ...
                v: consultaMotoristaOk
                  ? "Motorista consultado"
                  : "Consulta pendente",
              },
              {
                l: "Vr. Cliente",
                v: form.valorCliente
                  ? `R$ ${fmtNum(+form.valorCliente, { minimumFractionDigits: 2 })}`
                  : "",
              },
              {
                l: "Vr. Motorista",
                v: form.valorMotorista
                  ? `R$ ${fmtNum(+form.valorMotorista, { minimumFractionDigits: 2 })}`
                  : "",
              },
            ].map(
              (row) =>
  ...
  return (
    <div className="view">
        ...
        <div className="actions">
          {isEdit && (
            <button
              className="btn"
              onClick={() =>
                setStep(visibleSteps[visibleSteps.length - 1]?.n || 4)
              }
            >
      ...
      >
        <div
          style={{ display: "flex", borderBottom: "1px solid var(--divider)" }}
        >
        ...
        <div style={{ padding: 20 }}>
          {missingHints.length > 0 && (
            ...
            >
              <b style={{ color: "var(--text)" }}>Rascunho liberado.</b> Ainda
              faltam {missingHints.join(", ")}; a viagem pode ser salva e ficará
              como Faltando Dados.
            </div>
          )}
          {stepContent()}
        ...
        >
          <button
            className="btn"
            onClick={() =>
              currentStepIndex > 0 ? setStep(previousStep) : setMode("list")
            }
            style={{ minWidth: 100 }}
          >
            <Icon
