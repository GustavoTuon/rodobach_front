...
function cv2PrintTrip(viagem) {
  ...
  if (!popup) return;
  const loads = viagem.cargas.map((carga) => `<tr><td>${carga.codigo}</td><td>${carga.cliente}</td><td>${carga.origem}/${carga.ufOrigem} → ${carga.destino}/${carga.ufDestino}</td><td>${cv2Weight(carga.peso)}</td><td>${cv2Money(carga.valorCliente)}</td></tr>`).join("");
  popup.document.write(`<!doctype html><html><head><title>${viagem.numero}</title><style>body{font:14px Arial;color:#111;padding:32px}h1{font-size:24px;margin:0 0 4px}.meta{color:#555;margin-bottom:28px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:20px 0}.box{border:1px solid #ccc;padding:12px}.box span{display:block;color:#666;font-size:11px;text-transform:uppercase;margin-bottom:5px}table{width:100%;border-collapse:collapse;margin-top:16px}th,td{text-align:left;border-bottom:1px solid #ddd;padding:10px 6px}th{font-size:11px;text-transform:uppercase}.sign{margin-top:70px;display:grid;grid-template-columns:1fr 1fr;gap:80px}.sign div{border-top:1px solid #555;text-align:center;padding-top:7px}@media print{button{display:none}}</style></head><body><h1>Folha de viagem ${viagem.numero}</h1><div class="meta">Emitida em ${new Date().toLocaleString("pt-BR")}</div><div class="grid"><div class="box"><span>Data</span><b>${cv2Date(viagem.data)}</b></div><div class="box"><span>Veículo</span><b>${viagem.placa || "—"}</b></div><div class="box"><span>Motorista</span><b>${viagem.motorista || "—"}</b></div><div class="box"><span>Celular</span><b>${viagem.numeroMotorista || "—"}</b></div><div class="box"><span>CNH</span><b>${viagem.cnh || "—"}</b></div><div class="box"><span>ANTT / RNTRC</span><b>${viagem.antt || "—"}</b></div></div><h2>Cargas</h2><table><thead><tr><th>Carga</th><th>Cliente</th><th>Rota</th><th>Peso</th><th>Valor</th></tr></thead><tbody>${loads}</tbody></table>${viagem.observacoes ? `<h2>Observações</h2><p>${viagem.observacoes}</p>` : ""}<div class="sign"><div>Responsável pela programação</div><div>Motorista</div></div><script>window.onload=()=>window.print()</script></body></html>`);
...
function Cv2TripDetails({ viagem, onClose }) {
  return <Cv2Modal title={`Viagem ${viagem.numero}`} subtitle={`${cv2Date(viagem.data)} · ${viagem.placa || "Sem placa"}`} onClose={onClose} wide>
    <div className="cv2-modal-body">
      ...
      <div className="cv2-linked-loads">{viagem.cargas.map((carga) => <div key={carga.id}><div><strong>{carga.codigo} · {carga.cliente}</strong><span>{carga.origem}/{carga.ufOrigem} → {carga.destino}/{carga.ufDestino}</span></div><div className="cv2-linked-statuses"><Cv2Status value={carga.status} /><Cv2FinancialStatus value={carga.financeiro} /></div></div>)}</div>
    </div>
...
const CargasViagensV2 = ({ user }) => {
  ...
  return <div className="cv2">
    ...
    {tab === "cargas" ? <div className="cv2-kpis">
      <div className="cv2-kpi total"><span>Cargas cadastradas</span><b>{summary.cargas || 0}</b></div>
      <div className="cv2-kpi vehicle"><span>Veículo vinculado</span><b>{summary.cargas_vinculadas || 0}</b></div>
      <div className="cv2-kpi cte"><span>Aguardando CT-e</span><b>{summary.aguardando_cte || 0}</b></div>
      <div className="cv2-kpi transit"><span>Em trânsito</span><b>{summary.cargas_em_transito || 0}</b></div>
      <div className="cv2-kpi delivered"><span>Entregues</span><b>{summary.cargas_entregues || 0}</b></div>
      <div className="cv2-kpi paid"><span>Quitadas</span><b>{summary.quitadas || 0}</b></div>
    </div> : <div className="cv2-kpis">
      <div className="cv2-kpi total"><span>Viagens cadastradas</span><b>{summary.viagens || 0}</b></div>
      <div className="cv2-kpi cte"><span>Aguardando CT-e</span><b>{summary.viagens_aguardando_cte || 0}</b></div>
      <div className="cv2-kpi transit"><span>Em trânsito</span><b>{summary.viagens_em_transito || 0}</b></div>
      <div className="cv2-kpi delivered"><span>Entregues</span><b>{summary.viagens_entregues || 0}</b></div>
      <div className="cv2-kpi paid"><span>Quitadas</span><b>{summary.viagens_quitadas || 0}</b></div>
    </div>}
